"""
Vision Training Status Service
...
class VisionTrainingStatusService:
    """
    Service for handling real-time vision operations and training status updates.
    ...
    async def update_training_progress(self, session_id: str, progress: float, message: str) -> None:
        ...
        try:
            from main import broadcast_vision_training_update
            await broadcast_vision_training_update(session_id, {
                'progress': progress,
                'message': message,
    ...
    # Vision-specific operations can be added here
    async def get_vision_model_status(self) -> Dict[str, Any]:
        """
        Get current vision model status.

        Returns:
            Vision model status information
        """
        # This would integrate with the existing vision service
        return {
            'current_model': 'yolov8n',
            'available_models': ['base', 'colors', 'digits', 'merged'],
    ...
    async def get_training_data_stats(self) -> Dict[str, Any]:
        ...
        """
        # This would integrate with the existing vision training data management
        return {
            'total_images': 0,
            'annotated_images': 0,
...
vision_training_status_service = VisionTrainingStatusService()


async def get_vision_training_status(session_id: str = None) -> Dict[str, Any]:
    """
    Get vision training status - wrapper for the service.
    ...
    Returns:
        Training status
    """
    return await vision_training_status_service.get_training_status(session_id)


async def update_vision_training_progress(session_id: str, progress: float, message: str) -> None:
    """
    Update vision training progress - wrapper for the service.
    ...
    """
    await vision_training_status_service.update_training_progress(session_id, progress, message)


async def complete_vision_training(session_id: str, success: bool, message: str = None) -> None:
    """
    Complete vision training session - wrapper for the service.
    ...
    """
    await vision_training_status_service.complete_training_session(session_id, success, message)
